...
QUnit.test('TEST calcArea', assert => {
  assert.equal(calcArea(1, 1), 0.5, 'Positive integers')
  assert.equal(calcArea(-1, -1), 0.5, 'positive integers')
  assert.equal(calcArea(0,0), 0, 0)
  assert.equal(calcArea(10, 1), 5,5)
  assert.equal(calcArea(-10, 1), -5, 'Negative integers')
